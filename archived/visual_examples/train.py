...
def remove_black(img):
    """Remove black pixels and change them to white."""
    # img[img <= 20] = 255
    return img
...
def train(cfg_path = "./default.yaml", seed=None):
    ...
    vis_obs = env.render()
    vis_obs = cv2.cvtColor(vis_obs, cv2.COLOR_RGB2BGR)
    vis_obs = cv2.resize(vis_obs, (cfg.img_size, cfg.img_size), interpolation=cv2.INTER_LINEAR).transpose(2, 0, 1)
    vis_obs = remove_black(vis_obs)
    vis_obs = np.concatenate([vis_obs, vis_obs], axis=0) # stack with it self
    ...
    for step in range(0, cfg.train_steps+cfg.episode_length, cfg.episode_length):
        ...
        _, _ = env.reset(seed=cfg.seed+step)
        vis_obs = env.render()
        vis_obs = cv2.cvtColor(vis_obs, cv2.COLOR_RGB2BGR)
        vis_obs = cv2.resize(vis_obs, (cfg.img_size, cfg.img_size), interpolation=cv2.INTER_LINEAR).transpose(2, 0, 1)
        vis_obs = remove_black(vis_obs)
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var(), gpytorch.settings.max_root_decomposition_size(128):
            while not episode.done:
                ...
                for _ in range(cfg.action_repeat):
                    _, r, d, _, _ = env.step(action.detach().cpu().numpy())
                    raw_vis_obs = env.render()
                    raw_vis_obs = cv2.cvtColor(raw_vis_obs, cv2.COLOR_RGB2BGR)
                    reward += r # accumulate reward over action repeat
                    done = done or d
                    if done:
                        break
                # next_vis_obs = cv2.cvtColor(raw_vis_obs, cv2.COLOR_RGB2BGR)
                raw_vis_obs = cv2.resize(raw_vis_obs, (cfg.img_size, cfg.img_size), interpolation=cv2.INTER_LINEAR).transpose(2, 0, 1)
                raw_vis_obs = remove_black(raw_vis_obs)
                next_vis_obs = np.concatenate([vis_obs[3:], raw_vis_obs], axis=0) # stack two frames
                # Convet next_vis_obs = (3, 64, 128)
                display_obs = np.concatenate([vis_obs[3:], raw_vis_obs], axis=1).transpose(1, 2, 0)
                display_obs = cv2.resize(display_obs, (256, 512), interpolation=cv2.INTER_LINEAR)
                if cfg.render_eval:
                    cv2.imshow("env", display_obs)
        ...
        if step >= cfg.seed_steps:
            num_updates = (cfg.seed_steps if step == cfg.seed_steps else cfg.episode_length) // 2
