...
def train(cfg_path = "./configs/default.yaml", seed=None, kernel=None):
    ...
    for step in range(0, cfg.train_steps+cfg.episode_length, cfg.episode_length):
        ...
        common_metrics = {
            ...
            'num_inducing_points': agent.dynamics_gp.m_u.shape[-1] if cfg.kernel != "NA" else 0,
        }
    ...
    with open(f"./results/{cfg.task}/metrics_{cfg.exp_name}_{cfg.kernel}_{cfg.seed}.csv", "w") as f:
        f.write("episode,step,env_step,total_time,episode_reward,num_inducing_points\n")
        for i in range(episode_idx):
            f.write(f"{train_metrics['episode'][i]},{train_metrics['step'][i]},{train_metrics['env_step'][i]},{train_metrics['total_time'][i]},{train_metrics['episode_reward'][i]},{train_metrics['num_inducing_points'][i]}\n")
    print(f"Training metrics saved to ./results/{cfg.task}/metrics_{cfg.exp_name}_{cfg.kernel}_{cfg.seed}.csv")
...
if __name__ == "__main__":
    ...
    train(args.cfg_path, args.seed, args.kernel)